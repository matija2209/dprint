...
import logoImg from '@/assets/bauhar-logo-white.png';
  ...
  <Sheet>
    <SheetTrigger asChild>
      <Button 
        variant="ghost" 
        size="icon" 
        className={`fixed top-4 right-4 z-50 rounded-full w-10 h-10 flex items-center justify-center ${
      ...
      <div className="flex flex-col h-full">
        <div className="flex items-center justify-between px-4 py-3 border-b border-gray-100">
          <div className="flex items-center">
            <Image src={logoImgBlack} alt="Bauhar Logo" width={120} height={40} className="h-auto w-auto" />
          </div>
          <SheetClose asChild>
          ...
          <div className="space-y-1 px-2">
            {navLinks.map((link) => (
              <SheetClose asChild key={link.href}>
                <Link 
                  href={link.href}
                  className={`flex items-center px-3 py-3 rounded-lg text-base font-medium transition-colors relative group ${
                    pathname === link.href 
